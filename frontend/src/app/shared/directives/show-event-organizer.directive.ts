...
@Directive({
  selector: '[appShowEventOrganizer]',
})
export class ShowEventOrganizerDirective implements OnInit, OnDestroy {
  ...
  private checkRights(login: JwtLoginInformation) {
    ...
    const isContentManager = login.roles == Role.EventOrganizer;
