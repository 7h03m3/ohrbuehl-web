...
@Directive({
  selector: '[appShowOrganizationManager]',
})
export class ShowOrganizationManagerDirective implements OnInit, OnDestroy {
  ...
  private checkRights(login: JwtLoginInformation) {
    ...
    const isContentManager = login.roles == Role.OrganizationManager;
